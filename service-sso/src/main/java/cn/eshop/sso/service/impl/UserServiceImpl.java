package cn.eshop.sso.service.impl;

import cn.eshop.mapper.TbUserMapper;
import cn.eshop.pojo.TbUser;
import cn.eshop.pojo.TbUserExample;
...
public class UserServiceImpl implements UserService {
    ...
    public JDResult login(@RequestBody TbUser user) {

        if (user == null) {
            return JDResult.build(400, "error", "数据为空");
        }
    ...
    public JDResult token(String token, String callback) {
        ...
        if (StringUtils.isNotBlank(callback)) {
            return JDResult.ok(callback); // 未处理
        }
        ...
        return JDResult.build(400, "没有此用户");
    }
    ...
    public JDResult logout(String token, String callback) {
        ...
        try {
            ...
            return JDResult.build(400, "没有登录");
        }
