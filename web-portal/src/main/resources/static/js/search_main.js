...
GS.search = function () {
    ...
    searchlog(1, 0, 0, 26);
    window.open("/Search?q=" + encodeURIComponent(a) + "&enc=utf-8")
};
SEARCH.iframe_click = {
    ...
    html: null, init: function () {
        ...
        if ("string" == typeof (e) && e.length > 0) {
            ...
            for (var f = 0, d = Math.min(7, a.length); f < d; f++) {
                ...
                h += "<a onclick='searchlog(1, 0, " + f + ', 52, "' + b + "\");' href='Search?q=" + encodeURIComponent(b) + "&enc=utf-8'" + c + ">" + b + "</a>" + k
            }
            h += "</div>";
        ...
        modal: true, complete: function (c) {
            if (c != null && c.IsAuthenticated != null && c.IsAuthenticated) {
                ...
                $.getJSON(d, function (f) {
                    if (f.code == 1) {
                        var e = confirm("如您已安装JDLeBook客户端，请点击“确定”自动启动客户端\n如您尚未安装JDLeBook客户端，请点击“取消”将引导您免费安装客户端");
                        if (e == true) {
                            window.location = "LEBK:///Bought"
                        } else {
                            if (confirm('如果您没有安装LeBook客户端，请点击"确定"按钮到JD网站下载\n如果您不需要安装，请点击"取消"按钮')) {
